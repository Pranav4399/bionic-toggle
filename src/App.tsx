...
function BionicToggle({ className }: { className: string }): JSX.Element {
  ...
  const handleToggle = () => {
    const componentSelectors: string = componentNames.map((name) => `.${className} ${name}`).join(', ')
    const textTags: NodeListOf<HTMLElement> = document.querySelectorAll(componentSelectors)
      ...
      if (!isToggled) {
        ...
        textNodes.forEach((node: Node) => {
          ...
          // If the child node is a p or span tag
            ...
            const text: string = node.textContent as string
            const boldChildText: string | null = text ? boldWords(text).join(' ') : null
  ...
  const boldWords = (text: string): string[] => {
    ...
    return text
      ...
      .map((x: string) =>
        x.length === 1
          ? first + x + last
          : first + x.slice(0, Math.round(x.length / 2)) + last + x.slice(Math.round(x.length / 2)),
      )
  }
